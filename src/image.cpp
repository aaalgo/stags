...
namespace stags {
    ...
    public:
        ImageTagger (Config const &config) {
            string model = config.get<string>("stags.image.model", "models/image");
            string model_file   = model + "/model";
            string trained_file = model + "/trained";
            string mean_file    = model + "/mean";
            string label_file   = model + "/label";
            cfier = new Classifier(model_file, trained_file, mean_file, label_file);
            BOOST_VERIFY(cfier);
        }
        ...
        virtual void tag (string const &object, vector<Tag> *tags) {
            ...
            if (img.empty()) return;
            std::vector<Prediction> predictions = cfier->Classify(img);
            /* Print the top N predictions. */
            for (size_t i = 0; i < predictions.size(); ++i) {
                Prediction p = predictions[i];
                tags->emplace_back("OBJECT", p.first, p.second);
            }
        }
    };
